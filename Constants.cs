...
namespace MyPacMan
{
    static class Constants
    {
        ...
        private static int[] cruiseElroyTimers = { 20, 30, 40, 40, 40, 50, 50, 50, 60, 60 };
        ...
        private static int[] blueTimes = { 6, 6, 4, 3, 2, 6, 2, 2, 1, 5, 2, 1, 1, 3, 1, 1, 0, 1, 0, 0, 0 };
        public static int BlueTime()
        {
            return Level > blueTimes.Length - 2 ? 0 : blueTimes[Level - 1];
        }

        private static int[] bonusScores = { 100, 300, 500, 700, 700, 1000, 1000, 2000, 2000, 3000, 3000, 5000, 5000, 5000 };
        public static int BonusScores()
        {
            return Level > bonusScores.Length - 2 ? 5000 : bonusScores[Level - 1];
        }

        private static string[] bonusSprites = { "Cherry", "Strawberry", "Apple", "Bell", "Orange", "Pear", "Pretzel", "Bell", "Banana", "Key", "Key" };
        public static string BonusSprite()
        {
            return Level > bonusSprites.Length - 2 ? "Key" : bonusSprites[Level - 1];
        }

        private static int[] pacManSpeed = { 7, 9, 8, 8, 9 };
        public static int PacManSpeed()
        {
            if (5 <= Level && Level <= 20)
            {
                return pacManSpeed[4];
            }
            else if (5 > Level)
            {
                return pacManSpeed[Level - 1];
            }
            else
            {
                return 10;
        ...
        private static int[] ghostSpeed = { 13, 11, 12, 12 };
        public static int GhostSpeed()
        {
            return Level > 4 ? 11 : ghostSpeed[Level - 1];
        }
    }
