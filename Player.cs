...
namespace MyPacMan
{
    public class Player
    {
        ...
        Texture2D dyingFrames;
        Texture2D[] eatingFrames;
        int[] usedFramesIndex;
        int updatesPerPixel;
        int updateCount;
        Position position;
        ...
        public Player(Game game)
        {
            Reset();
            this.game = game;
            updatesPerPixel = Constants.PacManSpeed();
            spriteBatch = (SpriteBatch)game.Services.GetService(typeof(SpriteBatch));
            eatingFrames = new Texture2D[]
            {
            ...
            dyingFrames=game.Content.Load<Texture2D>("sprites/DyingSheetNew");
        }
        ...
        void TryMove()
        {
            ...
            else if ((position.Tile == new Point(0, 14) && direction == Direction.Left) ||
                     (position.Tile == new Point(27, 14) && direction == Direction.Right))
            {
                DoMove();
            }
            // If on a tile, we only move if the next tile in our direction is open
            else if ((direction == Direction.Up && Grid.TileGrid[position.Tile.X, position.Tile.Y - 1].Type == TileTypes.Open) ||
                      (direction == Direction.Down && Grid.TileGrid[position.Tile.X, position.Tile.Y + 1].Type == TileTypes.Open) ||
                      (direction == Direction.Left && Grid.TileGrid[position.Tile.X - 1, position.Tile.Y].Type == TileTypes.Open) ||
                      (direction == Direction.Right && Grid.TileGrid[position.Tile.X + 1, position.Tile.Y].Type == TileTypes.Open))
            {
                DoMove();
            }
        }
        void DoMove()
        {
            ...
            if (updateCount == 0)
            {
                ...
                {
                    updatesPerPixel = Constants.PacManSpeed() + 2;
                }
                else
                {
                    updatesPerPixel = Constants.PacManSpeed();
        ...
        public void Draw(GameTime gameTime, Vector2 boardPosition)
        {
            ...
            Vector2 pos;
            pos.X = boardPosition.X + (position.Tile.X * 16) + position.DeltaPixel.X - ((eatingFrames[0].Width - 16) / 2);
            pos.Y = boardPosition.Y + (position.Tile.Y * 16) + position.DeltaPixel.Y - ((eatingFrames[0].Height - 16) / 2);

            // At level start, just draw the full pac man and exit
            if (state == State.Start)
            {
                spriteBatch.Draw(eatingFrames[0], pos, Color.White);
            }
            ...
            else if (state == State.Normal)
            {
                ...
                int frame = Math.Abs(position.DeltaPixel.X + position.DeltaPixel.Y) / (16 / usedFramesIndex.Length);
                frame = (int)MathHelper.Clamp(frame, 0, usedFramesIndex.Length - 1);
                RenderSprite(eatingFrames[usedFramesIndex[frame]], null, boardPosition, pos);
            }
            ...
            else if (state == State.Dying)
            {
                ...
                int index = (timer.Seconds * 1000 + timer.Milliseconds) / timeBetweenFrames;
                if (index > 19)
                {
                    return;
                }
                RenderSprite(dyingFrames, new Rectangle(26 * index, 0, 26, 26), boardPosition, pos);
            }
        ...
        void RenderSprite(Texture2D spriteSheet, Rectangle? rectangle, Vector2 boardPosition, Vector2 position)
        {
            ...
            if (position.X < boardPosition.X)
            {
                ...
                spriteBatch.Draw(spriteSheet, leftPortionPosition, leftPortion, Color.White);
                spriteBatch.Draw(spriteSheet, rightPortionPosition, rightPortion, Color.White);
            }
            // Next, to the right
        ...
        void Reset()
        {
            ...
            usedFramesIndex = new int[] { 0, 1, 2 };
            position = new Position { Tile = new Point(13, 23), DeltaPixel = new Point(8, 0) };
            updateCount = 0;
        }
        /// <summary>
        ...
        void TryTurn(Keys input)
        {
            ...
            if (position.DeltaPixel != Point.Zero)
            {
                if ((direction == Direction.Up && input == Keys.Down) ||
                    (direction == Direction.Down && input == Keys.Up) ||
                    (direction == Direction.Left && input == Keys.Right) ||
                    (direction == Direction.Right && input == Keys.Left))
                {
            ...
            else if ((input == Keys.Up && Grid.TileGrid[position.Tile.X, position.Tile.Y - 1].Type == TileTypes.Open) ||
                      (input == Keys.Down && Grid.TileGrid[position.Tile.X, position.Tile.Y + 1].Type == TileTypes.Open) ||
                      (input == Keys.Left && Grid.TileGrid[position.Tile.X - 1, position.Tile.Y].Type == TileTypes.Open) ||
                      (input == Keys.Right && Grid.TileGrid[position.Tile.X + 1, position.Tile.Y].Type == TileTypes.Open))
            {
                DoTurn(input);
            }
